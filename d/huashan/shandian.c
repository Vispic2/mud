...
int do_decide(string arg)
{
	...
	if(	me->query_skill("jiuyin-shengong",1)< 600+skills&& me->query_skill("bluesea-force",1)< 600+skills&& me->query_skill("never-defeated",1)< 600+skills&& me->query_skill("kuihua-mogong",1)< 600+skills)
		{
		msg=msg+"jiuyin-shengong转世要求"+(600+skills)+",当前："+(me->query_skill("jiuyin-shengong",1))+"\n";
	
		msg=msg+"bluesea-force转世要求"+(600+skills)+",当前："+(me->query_skill("bluesea-force",1))+"\n";
	
		msg=msg+"never-defeated转世要求"+(600+skills)+",当前："+(me->query_skill("never-defeated",1))+"\n";
	
		msg=msg+"kuihua-mogong转世要求"+(600+skills)+",当前："+(me->query_skill("kuihua-mogong",1))+"\n";
	
	return notify_fail(msg+"你至少要有一门宗师级能达到"+(600+skills)+"级以上才能转世重修！\n");
	}
...
void jieti_msg(object me, int stage, string place)
{
	...
	} else
	{
		...
		me->set("mud_age",1036800);//86400*3*4=18岁s
