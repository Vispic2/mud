...
LONG
	...
	set("item_desc", ([
		"【青砖】": "几块普通的青砖，看不出有什么特别的。\n"
			ZJOBACTS2+ZJMENUF(2,2,9,30)"敲打敲打:ci brick"ZJSEP"粘出来:zhan brick\n",
	]) );
...
int do_ci(string arg)
{
	...
	if (!objectp(weapon = me->query_temp("weapon")))
		return notify_fail("用手敲？不嫌手疼么？找个家伙敲吧！\n");

	message_vision("$N拔出兵刃，向青砖轻轻刺去，只听得空空作声，青砖的另一头仿佛是空的。\n",this_player());
...
int do_zhan(string arg)
{
	...
	message_vision("$N双手紧贴青砖，使个‘粘’字诀，轻轻巧巧地将这块青砖粘了出来。如此没粘到几块砖，墙壁上就现出了一个大洞。\n", this_player());
