...
void create()
{
	set("short", "赌场");
	set("long", @LONG
大厅里摆满大大小小的赌桌，只要能说出名的赌具，这里都有。进
来这里你首先听到的就是少女们吃吃的笑声和男人们的吹牛声，掷骰子
声……在这里，你几乎就可以听到世上所有不规矩的声音。墙上挂着一
块匾（ｓｉｇｎ）。
LONG);
	set("exits", ([ /* sizeof() == 1 */
...
kind:
    头彩 <tc>（骰数由每次开盘前确定）      ：一赢二十五
    双对 <sd>（两骰号相同，且为偶数）      ：一赢十
    七星 <qx>（两骰之和为七）	      ：一赢五
    散星 <sx>（两骰之和为三，五，九，十一）：一赢二
TEXT
	]));
