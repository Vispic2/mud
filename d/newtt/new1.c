...
int do_email(string arg)
{
	...
	if(!arg){	
	return notify_fail(INPUTTXT("请务必输入正确的QQ邮箱，稍后会给你发一封注册验证码","email $txt#")+"\n");	
	}
	write("设置邮箱成功，你设置的邮箱是："+arg+"\n");
...
int do_tianxing(string arg)
{
	...
	if (arg == "j")
	{
		message_vision(HIY"你选择了天性为：狡黠多变。\n"NOR, me);	    
	...
	if (arg == "g")
	{
		message_vision(HIY"你选择了天性为：光明磊落。\n"NOR, me);	    
	...
	if (arg == "y")
	{
			message_vision(HIY"你选择了天性为：阴险狡诈。\n"NOR, me);	    
	...
	if (arg == "x")
	{
		message_vision(HIY"你选择了天性为：心狠手辣。\n"NOR, me);	    
...
int do_washto(string arg)
{
	...
	if(tmpstr<10||tmpint<10||tmpcon<10||tmpdex<10||tmpstr>50||tmpint>50||tmpcon>50||tmpdex>50||(tmpstr+tmpint+tmpcon+tmpdex)!=80)
	{
		tell_object(me,INPUTTXT("输入格式不对，请重新输入你想分配的方案：$br#"
						"按照臂力、悟性、根骨、身法次序输入4个用"
						"空格分开的数，注意每个数不能低于13或大于30，4个数之和必须为80。","washto $txt#")+"\n");
