...
int accept_object(object me, object ob)
{
	...
	if (n < cost)
	{
		...
		if (n * 2 >= cost && me->query("special_skill/treat"))
			message_vision("$N跪在地上，抱着$n，哭喊道：“你就可怜可怜我这穷鬼吧！”\n"
						"$n看了，颇为无奈，摇摇头，叹口气道：“好吧好吧，算了。”\n", me, this_object());
		else
			return 0;
	}

	message_vision("$n接过$N递过来的" + ob->name() + "，记在帐上，有道：“等会儿！”\n",me, this_object());
	destruct(ob);
	message_vision("$N高声喊道：“" + helper->name() + "！快来，快来啊，送钱的上门了！”\n", this_object());
	helper->move(environment());
	message_vision("$n走了过来，看了看$N。\n", me, helper);
	command("say 好啦，" + helper->name() + "，就是这位" +	RANK_D->query_respect(me) + "，你一同去吧。");
	...
	CHANNEL_D->do_channel(this_object(), "rumor",me->name(1) + "在南海邀请到了一名侠客岛弟子。");
	return -1;
}
...
mixed accept_ask(object me, string topic)
{
	...
	message_vision(CYN "$N" CYN "看了看$n" CYN "，疑问道：“你想干什么？是要找我的" HIY "同门"NOR CYN "师兄弟么？”"NOR"\n",this_object(), me);
	return 1;
}
...
mixed try_to_hire(object me, object ob)
{
	...
	message_vision("$N微微一笑，道：“好吧，不过你也得意思意思啊，我们侠客岛开销也不小的。”\n"
		       "$N仔细考虑了一会儿，对$n道：“这样吧，就" + chinese_number(cost) + "两黄金吧。 ”\n",
		       this_object(), me);
