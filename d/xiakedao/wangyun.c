...
LONG    );
	...
	set("item_desc", ([
		"【小草】" : "几珠碧绿的小草，你不禁想拿(take)起来。\n"
		ZJOBACTS2"拿下:take cao\n",
	]));
...
int do_take(string arg)
{
	...
	} else
	{
		...
		message_vision(HIR "只见$N" HIR "的身子在空中打了几个盘旋，呀！不好！！！"NOR"\n",
			       me);
		me->move(__DIR__"gudi");
		tell_object(me,HIR "你从悬崖上摔了下来，浑身疼痛，受了好几处伤。"NOR"\n");
		message("vision", HIR "只见" + me->name() + HIR "你从悬崖上摔了下来，躺在地上，半天不动。"NOR"\n", environment(me), me);
	}
	return 1;
}
