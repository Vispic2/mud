...
int jjsign()
{
	...
	if(localtime(now)[6]!=LUNJIAN_DAY||localtime(now)[2]!=LUNJIAN_HOUR||localtime(now)[1]>LUNJIAN_PRE)
		return notify_fail(ZJOBLONG"报名时间：周日晚"+LUNJIAN_HOUR+":00-"+LUNJIAN_HOUR+":10。\n");

	if (me->query("combat_exp")<200000)
		return notify_fail("你经验尚浅(20w)，还是把功夫练好再来吧。\n");

	if (me->query_skill("force")<180)
		return notify_fail("华山论剑起码要有180内功激发，你还是把功夫练好再来吧。\n");
	...
	if(!sizeof(rooms))
	{
		...
		if(sizeof(us)<1 || turn >= LUNJIAN_TURN)
		{
			...
			else
			{
				...
				msg = "华山论剑圆满结束，" + name + "获得【无双】的称号！";
				FAMILY_D->set_lunjian_leader("无双",total[0]);
				message_competition(msg);
					...
					if(i==0)
					{
						if(ob = find_player(total[0]))
						{
						   ob->add("combat_exp",300000);
							ob->add("yuanbao",1000);
							tell_object(ob,"你在本次华山论剑中获得官方奖励300000，1000元宝。\n");
					...
					else if(i>0 && i<5)
					{
						if(ob = find_player(total[i]))
						{
						   ob->add("combat_exp",200000);
							ob->add("yuanbao",500);
							tell_object(ob,"你在本次华山论剑中获得官方奖励200000，500元宝。\n");
						}
					}
					else
					{
						if(ob = find_player(total[i]))
						{
							...
							tell_object(ob,"你在本次华山论剑中获得官方奖励100000经验，10000潜能\n");					
						}
					}
				}
