...
//tell_object(find_player("name"),"节点触发\n");
if((query_temp("quester")!=getuid(ob))||(query_temp("quester_bs")!=getuid(ob))){
ob->add("quest/kill_npc",1);
//tell_object(ob,""HIR"禁止恶意击杀他人师门任务NPC已将你转移回扬州客栈，你的行为已被系统记录超过一定数量自动检测删号处理！"NOR"\n");
...
void do_continue_help(object ob)
{
	...
	while (n--)
	{
		...
		obs[n]->set_temp("quester_bs",ob->query("id"));
	}
...
void random_place(string msg)
{
	...
	// 初始化并记录这个任务
	//tell_object(find_player("name"),sprintf("执行结果：%O 类型："+typeof(msg)+"\n",msg));
//	tell_object(find_player("name"),"执行结果："+msg+" 类型："+typeof(msg)+"\n");
	...
	if ((n = sizeof(obs)) > 0)
	{
		...
		foreach (hob in obs)
		{
			...
			QUEST_D->delay_bonus(hob, 
				([ "exp" : exp + random(4) + ((hob == dob) ? extra_exp : 0),
		...
		/*
			...
			{
				// 调用延迟奖励，为什么这么调用？很简单，
				// 因为我希望玩家在看到NPC挂了以后才能够
				// 看到奖励。
				if(hob->query("shimen/today")<10000){
					QUEST_D->delay_bonus(hob, 
						([ 
							"exp" : exp + random(4),// + ((hob == dob) ? extra_exp : 0),
							...
							"weiwang" : weiwang + random(3),
							"score"   : score + random(3),
							"prompt"  : "协助 "+qob->name(1)+" 击毙" + name() + "之后" ]));
				}
			}
			*/
	}
