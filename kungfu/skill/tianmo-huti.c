...
int valid_learn(object me)
{
	if ((int)me->query_skill("force") < 500)
		return notify_fail("你的内功火候不够，难以修习这顶级武学。\n");

	if ((int)me->query("max_neili") < 3000)
		return notify_fail("你的内力还不足以修习这顶级武学。\n");

	if ((int)me->query_skill("force", 1) < (int)me->query_skill("tianmo-huti", 1))
		return notify_fail("你的基本内功水平有限，无法领会更高深的天魔护体。\n");
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
	...
	if (! living(me) ||
	    (int)me->query_skill("tianmo-huti", 1) < 100 ||
	...
	{
		me->add("neili", -cost);
            tell_object(me,HIR"你使出天魔护体，用"+cost+"内力抵消了"+damage+"伤害。"NOR"\n"NOR);
...
int practice_skill(object me)
{
	...
	if ((lvl = me->query_skill("tianmo-huti", 1)) < 180)
	...
	if ((int)me->query("neili") < cost)
		return notify_fail("你的内力不够练顶级武学。\n");
...
{
	return __DIR__"tianmo-huti/" + action;
}
