...
int valid_learn(object me)
{
	...
	lvl = me->query_skill("tianmo-bufa", 1);
	if (lvl > 300) lvl = 300;
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
	...
	if ((int)me->query_skill("tianmo-bufa", 1) < 100 ||
	    ! living(me))
		return;
...
int query_effect_dodge(object attacker, object me)
{
	...
	if (lvl < 150) return 130;
	if (lvl < 200) return 140;
	if (lvl < 250) return 150;
	if (lvl < 300) return 160;
...
int practice_skill(object me)
{
	...
	cost = me->query_skill("tianmo-bufa", 1) / 2 + 100;
	if ((int)me->query("qi") < 500)
		return notify_fail("你的体力太差了，难以练习天魔步法。\n");

	if ((int)me->query("neili") < cost)
		return notify_fail("你的内力不够了，无法练习天魔步法。\n");
...
{
	return __DIR__"tianmo-bufa/" + action;
}
