...
{
    int i, level;
    level = (int)me->query_skill("hubo", 1);
...
int practice_skill(object me)
{
   ...
   return notify_fail("你的内力不够练左右搏击之术。\n");
    if ((int)me->query_skill("hubo",1) < 20)
   return notify_fail("以你目前的功力尚不够自己练习。\n");
    me->receive_damage("qi", 60);
    me->receive_damage("jing", 30);
    switch( random(3) ) {
    case 0:
    me->add("combat_exp", -random((int)me->query_con() + (int)me->query_skill("hubo",1) )/20);
    write( HIR "你只觉得心绪一阵烦乱，头一阵发晕，你的经验降低了！\n" NOR);
    break;
    case 1:
    me->add("potential", random((int)me->query_con() + (int)me->query_skill("hubo",1) )/20);
    me->add("combat_exp", random((int)me->query_con() + (int)me->query_skill("hubo",1) )/20);
