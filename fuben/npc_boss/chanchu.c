...
void random_move()
{
        ...
        {                
            ...
            message("channel:chat",CYN"【BO"HIG"SS】"HIG"："+query("name")+HIG+"长啸一声，化做一滩血水，渗入地下。"NOR"\n",users());	      
...
ob->move(environment());
message_sort(HIW "$N" HIW "长啸一声，留下了一物你定眼一看原来是"+ob->name()+HIW+"，随后变化做一滩血水，渗入地下。"NOR"\n", this_object());
}
if(a==5)
{
...
message_sort(HIW "$N" HIW "长啸一声，变化做一滩血水，渗入地下\n", this_object());
message("channel:chat",CYN"【BO"HIG"SS】"HIG"："+query("name")+HIG+"长啸一声，变化做一滩血水，渗入地下。"NOR"\n",users());	
