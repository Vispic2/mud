...
int main(object me, string arg)
{
	...
	if ((int)me->query("max_neili") < 5000)
		return notify_fail("你觉得内力颇有不足，看来目前还难以进行长时间的闭关修炼。【需要5000点内力】\n");
...
int closing(object me)
{
	...
	if ((random(100) < 1) && me->can_improve_neili())
	{
		tell_object(me, HIR "你对内功有所领悟，感到内力进步了！"NOR"\n");
		me->add("improve/neili", 5);
		me->improve_neili(20 + random(20));
	}

	if ((random(100) < 1) && me->can_improve_jingli())
	{
		tell_object(me, HIM "你对神通有所领悟，感到精力进步了！"NOR"\n");
		me->add("improve/jingli", 5);
		me->improve_jingli(20 + random(20));
	}

	exp_inc = 100 + random(100);
	exp_inc = exp_inc * (100 + me->query_skill("martial-cognize", 1)) / 100;
	exp_inc = exp_inc * (100 + me->query("int")) / 100;
	me->add("combat_exp", exp_inc);
	me->improve_skill("martial-cognize", 1000 + random(1000));
	...
	if (r = sizeof(ks))
	{
		...
		me->improve_skill(ks[r], 10000 + random(1000));
	}
