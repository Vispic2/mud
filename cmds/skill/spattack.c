...
int main(object me, string arg)
{
	...
	if (couple->is_ghost())
		return notify_fail(couple_name + "现在是鬼魂一个，无法穿越灵界给与你力量。\n");
...
void do_remote_fight(object me, object couple, object target, int n)
{
	...
	{
		write("你试图和你的伴侣进行心灵上的沟通，但是一时觉得精力不济，难以联系。\n");
		return;
	}
	...
	if (weapon && (chinese_type = weapon_name[weapon->query("skill_type")]))
	{
		if (me->query("gender") == "女性")
			...
			msg = HIC "$N持" + chinese_type + "而进，竟然视$n" HIC "有若无物，手中" +
			      ...
			      chinese_type + "影，将$p" HIC "困在当中。"NOR"\n";
	} else
	{
		if (me->query("gender") == "女性")
			msg = HIG "$N" HIG "轻声浅笑，飘然而进，身法惊艳"
			      "绝伦，有若九天玄女踏足红尘，举手投足之间没有任何破绽。"NOR"\n";
		else
