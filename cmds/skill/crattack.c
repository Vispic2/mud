...
int main(object me, string arg)
{
	...
	if (! living(target))
		return notify_fail("人家都已经这个样子了，你还用得着这么费力气么？\n");
	...
	if (me->is_most_craze())
	{
		...
		if (me->query("gender") == "女性")
			msg = HIR "$N" HIR "眼神犹如利刃，冷冷的扫过$n" HIR "，让他不禁打了个寒战。只见$P低声喝"
			      "道：“" + RANK_D->query_rude(target) + "，你去死吧！”"NOR"\n";
		else
		...
		msg = HIR "$N" HIR "一招攻出，竟不罢休，随即舍身扑上又是一招！"NOR"\n";
		if (! me->is_fighting(target) && living(target))
		{
			msg += HIY "$n" HIY "见$N" HIY "舍生忘死，全然不顾自己已经认输做罢，不由大吃一惊，慌忙抵挡。"NOR"\n";
		}
		message_combatd(msg, me, target);
	...
	if (me->query("gender") == "女性")
		msg = HIR "$N" HIR "一声娇嗔，遥指$n" HIR "喝道：“" +
		      RANK_D->query_rude(target) + "，看招！”"NOR"\n";
	else
...
void hate_attack(object me, object target)
{
	...
	message_combatd(HIR "\n$N" HIR "叫道：好！你再接我左一招！"NOR"\n", me);
	COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 0);

	message_combatd(HIR "\n$N" HIR "叫道：休走！你再接我右一招！"NOR"\n", me);
	COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 0);

	message_combatd(HIR "\n$N" HIR "叫道：看清楚我这当中穿心一招！"NOR"\n", me);
	damage += me->query("str");
	attack += me->query("str");
	    ...
	    target->query("jing") >= 0)
	{
		if (random(2))
			shout_message(target->name(1) + "呵呵大笑道：这位" +
				      RANK_D->query_respect(me) + "，你好厉害啊！不过又岂能奈我何？");
		else
			shout_message(target->name(1) + "哈哈大笑道：你的" +
				      "来势好凶啊，可惜本事太差，也罢，也罢，哈哈！");

		tell_object(me, HIR "你目眦俱裂，心中燃起的重重怒火，丝毫没有减弱。"NOR"\n");
	} else
	{
		if (random(2))
		...
		tell_object(me, HIC "你长舒一口气，心情平静了许多。"NOR"\n");
	}
	me->start_busy(2);
}
...
void shout_message(string msg)
{
	message("channel:chat", HIR "【报仇" HIW "雪" HIR "恨】" + msg + ""NOR"\n",all_interactive());
}
