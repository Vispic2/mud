...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		...
		msg = ZJOBLONG"以下是你目前组合中的特殊拳术技能("+HIC+ZJSIZE(24)+ZJURL("cmds:prepare none")+"取消准备)"NOR+ZJBR;
		for (i = 0; i < sizeof(skill); i++)
		{
			if (! me->query_skill(skill[i]) ||
			    ! pmap[skill[i]]) continue;
			msg += valid_types[skill[i]] + " (" + skill[i] + ")" +"   " + to_chinese(pmap[skill[i]]) + ZJBR;
		}
		write(msg);
		return 1;
	}
	...
	if (arg == "?") 
	{
		msg = "以下是可以使用特殊拳术技能的种类("+HIC+ZJSIZE(24)+ZJURL("cmds:prepare none")+"取消准备)"NOR"\n";
