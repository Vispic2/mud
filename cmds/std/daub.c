...
int main(object me, string arg)
{
	...
	if (dest == me)
	{
		...
		tell_object(me, HIG "你把" + obj->name() + HIG "涂抹到自己手上。"NOR"\n");
	...
	message("vision", sprintf("%s拿出一些东西涂抹在%s上面。\n",
				  me->name(), dest->name()),
			  environment(me), ({ me }));
	tell_object(me, HIG "你把" + obj->name() + HIG "涂抹到" + dest->name() + HIG "上。"NOR"\n");
...
void check_poison(object me, object dest, int iknow)
{
	...
	if (lvl < 100 || lvl < (int)p["level"])
	{
		message("vision", HIC "忽然" + me->name() + HIC "眉头"
				  "紧缩，神情痛苦，看来是遇到麻烦了。"NOR"\n",
				  environment(me), ({ me }));
		tell_object(me, HIC "忽然你觉得有点不对劲，不好，可能是中毒了。"NOR"\n");
		me->affect_by(type, p);
	...
	if (p["level"] > 120)
	{
		if (iknow)
			...
			tell_object(me, HIC "你发现这" + dest->name() + HIC "上的" +
					name + HIC "毒性甚"
					"是猛烈，亏得你内功高深，才幸免无事。"NOR"\n");
	} else
	if (! iknow && dest->query_temp("who_id") != me->query("id"))
	{
		tell_object(me, HIC "你发现这" + name + HIC "上面带毒。"NOR"\n");
	}
}
