	...
	if (arg == "cancel")
	{
		...
		if (objectp(target_ob = present(target, environment(me))))
			tell_object(target_ob,
				    YEL + me->name(1) + "打消向你购买东西的念头。"NOR"\n");
		tell_object(me, YEL "你打消了交易的念头。"NOR"\n");
...
int do_purchase(object me, string from, string item, int count, string money)
{
	...
	me->set_temp("pending/purchase_info",
			...
			"money"  : money,
			"count"  : count ]));
	tell_object(me, YEL + "你向" + from_ob->name(1) + "提出了要求。"NOR"\n");
	return 1;
}
...
int do_right(object me, object ob)
{
	...
	{
		tell_object(me, "人家现在接不下这么多" + item_ob->name() + "。\n");
		tell_object(ob, "你身上的东西太多了，现在接不下那么多" + item_ob->name() + "。\n");
		return 1;
	}
