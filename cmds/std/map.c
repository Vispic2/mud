...
*/
	...
	{
		write(HIR"只有在户外才有必要绘制地图。\n"NOR);
		return 1;
	}

	if (! stringp(name) || clonep(env))
	{
		write(HIY"这里是一处神秘的地方，你无法判断它的方位。\n"NOR);
		return 1;
	}

	if (me->query("map_all"))
	{
		write(HIY"你已经获得了地图全集，没有必要再绘制地图了。\n"NOR);
		return 1;
	}

	if (me->is_busy())
	{
		write(HIW"你现在正忙，没有时间绘制地图。\n"NOR);
		return 1;
	}
	...
	{
		write(YEL"你已经绘制过这附近的地图了，没有必要再重复了。\n"NOR);
		return 1;
	}

	if (! MAP_D->been_known(here))
	{
		write(HIG"你觉得这里没什么好画的。\n"NOR);
		return 1;
	}
	...
	if (prompt = env->query("no_map"))
	{
		if (stringp(prompt))
			...
			write(HIR"你看了半天，也没有弄清楚附近的地形。\n"NOR);
		return 1;
	}

	if (me->query("jing") < 50)
	{
		write(HIG"你的精神不佳，无法集中全神贯注的绘制地图。\n"NOR);
		return 1;
	}
	...
	{
		write(HIR"你在纸上涂抹了一会儿，连自己都看不出是什么东西。\n"NOR);
		write(HIR"天神向你千里传音：你的绘画技巧不足30级，你可以向扬州北的鲁班学习此项技能。\n"NOR);
		return 1;
	}
	...
	msg = ""HIY"你获得了" + chinese_number(exp) + "点经验"NOR"";
	if (pot > 0)
		msg += ""HIY"和" + chinese_number(pot) + "点潜能"NOR"";

	if (score)
		msg += "，"HIY"通过游历，你累积了" + chinese_number(score) + "点江湖阅历"NOR"";
