...
int main(object me, string arg)
{
	...
	if (! arg || arg == "")
	{
		...
		if (stringp(old_target))
			write("你正守住往" + default_dirs[old_target] +
			      "的方向，不准 " + me->query_temp("guardfor_id") +
			      " 的离开。\n");
		else
	...
	if (objectp(ob))
	{
		...
		if (ob->query("no_guard"))
			message_vision("$n看起来不愿意接受$N的保护"
				       "。\n", me, ob);
		else
		if (living(ob))
			message_vision(HIW"$N开始保护$n。"+NOR+"\n", me, ob);
		else
	...
	} else
	{
		...
		tell_object(me, "你开始守住往" + default_dirs[arg] +
				"的方向不让 " + me->query_temp("guardfor_id") +
				" 通行。\n", me);
		me->set_temp("guardfor", arg);
	}
...
{
	write(@TEXT
指令格式：guard [<某人>|<地上某物>|<某个出口> for <某人>|cancel]
...
guard <某个出口>..守住某个出口防止指定的人从这个出口离开，可
		  以用来阻拦善于逃跑的敌人或拦路打劫做坏事，
		  如果你当时正忙，则不能阻挡别人。
