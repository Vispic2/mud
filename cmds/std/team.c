...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		if (! pointerp(t = me->query_team()))
			return notify_fail(ZJOBLONG+HIG"你现在并没有参加任何队伍。"NOR"\n");

		str = ZJOBLONG"你现在队伍中的成员有：\n"ZJOBACTS2+ZJMENUF(3,3,9,30);
		text = ({});
		for(i=0;i<sizeof(t);i++)
		{
			...
			if (me->is_team_leader())
			{
				if(i==0)
				{
					text += ({ "解散队伍:team dismiss" });
					text += ({ "调整阵型:team form" });
				}
				else
				{
					text += ({ "踢出队伍:team dismiss "+file_name(t[i]) });
					text += ({ "队长移交:team cleader "+file_name(t[i]) });
				}
			}
		}
		if(!me->is_team_leader())
			text += ({ "离开队伍:team dismiss" });
