...
int main(object me, string arg)
{
	...
	if (! ob->is_character())
	{
		message_vision("$N痴痴的望着$n自言自语：嫁给我吧...嫁给我吧...看来是疯了。\n", me, ob);
		return 1;
	}
	...
	if (me->query("class") == "bonze"&&( me->query("family/family_name") == "峨嵋派"|| me->query("family/family_name") == "少林寺"|| me->query("family/family_name") == "天龙寺"))
	{
		message_vision("$N对$n哼哼道：我也要结婚，我也要结婚！\n", me, ob);
		return 1;
	}

	if (ob->query("class") == "bonze"&&( ob->query("family/family_name") == "峨嵋派"|| ob->query("family/family_name") == "少林寺"|| ob->query("family/family_name") == "天龙寺"))
	{
	...
	if (me->query("couple"))
		return notify_fail("你可要稳住！根据我朝律法第一百条，重婚者将被打入地狱！\n");

	if (ob->query("couple"))
		return notify_fail("你怎么也得等人家离婚了再说吧？\n");

	if ((time()-me->query("divorce_time"))<86400*7 )
		return notify_fail("你这么快就另觅新欢了？也太喜新厌旧了吧！\n");

	if ((time()-ob->query("divorce_time"))<86400*7 )
		return notify_fail("对方离婚没多久，尚无心情马上再婚！\n");
	...
	tell_object(ob, YEL + me->name(1) + "在向你求婚呢，你答应(right)还是不答应(refuse)？"NOR"\n");
