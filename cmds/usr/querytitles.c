...
/*
	   ...
	   if (!arg)
   {
	...
	msg+=ZJOBACTS2+ZJMENUF(1,1,7,33); 	
	foreach (title in keys(ss)){		
		fmsg =HIC"["+title+"]"NOR;
		xg=TITLE_D->query_xg(me,title);
		yq=TITLE_D->query_yq(me,title);
		fmsg+=(me->query("titles/"+title)?HIW"["WHT"已激活"HIW"]"NOR"":""HIW"["WHT"未激活"HIW"]")+(me->query("chenghao")==title?"["WHT"已佩戴"HIW"]":""HIW"["WHT"未佩戴"HIW"]")+NOR+ZJBR+WHT+"激活要求："+NOR+xg+ZJBR+WHT+"奖励："+yq+NOR;
		if(me->query("titles/"+title))
		msg+=fmsg+":usetitles "+title+ZJSEP;	
		else
		msg+=fmsg+":querytitles buy=="+title+ZJSEP;	
	 }	
		msg += "\n";	
      	write(msg+NOR+"\n");
     	return 1;
