...
int main(object me, string arg)
{
	...
	//	str += "技能商店:myshop type 技能类"ZJSEP;
	//	str += "丹药商店:myshop type 丹药类"ZJSEP;
	//	str += "装备材料:myshop type 装备类"ZJSEP;
	...
	else if (sscanf(arg,"buy %s",file)==1)
	{
		...
		if(yuanbao)
		{
			...
			log_file("cmds/myshop", sprintf("%s:%s(%s)消费%d元宝，从随身商城购买%d个%s(%s)。\n",
		...
		else
		{
			...
			log_file("cmds/myshops", sprintf("%s:%s(%s)消费%s，从随身商城购买%d个%s。\n",
