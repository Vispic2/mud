...
int main(object me, string arg)
{
	...
	if (! arg)
	{
		if (objectp(ob = me->query_temp("quest/assist")))
			write("你现在正帮助" CYN + ob->name() + NOR "完成门派的任务。\n");
		else
	...
	if (arg == "cancel" || arg == "none")
	{
		...
		if (environment(ob) == environment(me))
			message_vision("$N挥了一挥手，对$n道：“我还有事，先不帮你了，改日再见！”\n",me, ob);
	...
	{
		write("你和" + ob->name() + "并不是同一个门派的呀，看来这份好意只能让对方心领了。\n");
		return 1;
	}
	...
	{
		write("你想帮助" + ob->name() + "，可是人家还没有答应你呢。\n");
		return 1;
	}

	if (me->query("quest"))
	{
		write("你现在正有任务在身，还是先忙完自己的事情再说吧！\n");
		return 1;
	}

	write("你决定开始帮助" + ob->name() + "完成任务。\n");
	message_vision("$N对$n道：“" + ob->name() + "，我和你一起去吧！”\n", me, ob);
	me->set_temp("pending/assist", ob);
	if (! playerp(ob))
	{
		message_vision("$n对$N说：“我还有别的事情哦，不用你帮忙啦！”\n", me, ob);
		return 1;
	}

	//tell_object(ob, YEL + me->name() + YEL "决定帮助你一同完成任务，你是否同意(right|refuse " + me->query("id") + ")？"NOR"\n");
	tell_object(ob, YEL + me->name() + YEL "决定帮助你一同完成任务："+ZJURL("cmds:refuse "+ me->query("id"))+ZJSIZE(20)+"拒绝"+NOR+"   "ZJURL("cmds:right "+ me->query("id"))+ZJSIZE(20)+"同意"+NOR+"\n");
...
int do_right(object me, object ob)
{
	...
	tell_object(ob, me->name() + "愿意接受你的帮助。\n");
	...
	if (! pointerp(me->query_team()) || me->is_team_leader())
	{
		...
		{
			write("你这个队伍中的人太多了，没法邀请" + ob->name() + "加入，这样让他帮助就不方便了。\n");
			return 1;
		}
		...
		tell_object(me, "让" + ob->name() + "帮助你最好是和你在一个队伍中，如果可以的话，就邀请对方组队吧！\n");
...
int do_refuse(object me, object ob)
{
	...
	ob->delete_temp("pending/assist");
	message_vision("$N摇摇头，对道：“你的好意我心领了，不过现在还不需要劳动大驾。”\n", me, ob);
	return 1;
}
