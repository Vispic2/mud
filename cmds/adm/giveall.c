...
int main(object me, string arg)
{
        ...
        tname = target->query("name", get_object(target));
        ...
        foreach(object player in users())
        {
                ...
                if (time() -  player->query_temp("logon_time") < 10) {
                        tell_object(player, HIW "它对着你清鸣几声，似乎是在说：很遗憾，你连线短于30分钟。\n"HIR"火凤"HIW"在你头上盘旋几圈，然后径直飞走了。\n\n" NOR);
                        continue;
                }
     if (player->query("combat_exp") < 1000000) {
                        tell_object(player, HIW "它对着你清鸣几声，似乎是在说：很遗憾，你的经验不足100w系统默认为小号。\n"HIR"火凤"HIW"在你头上盘旋几圈，然后径直飞走了。\n\n" NOR);
                        continue;
                }
                if (!player->query("family/master_id") || player->query("family/master_id")=="") {//还未拜师
                ...
                if( count>1 && ob->query("base_unit", ob) )
                        ob->set_amount(count);
