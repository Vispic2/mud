...
int main(object me, string arg)
{
	...
	if (! update_dir(me, "/data/login/", 2, total))
	{
		write(HIR "整理玩家登陆数据遇到错误中止。"NOR"\n");
	}
	else
		write(HIR "玩家登陆数据整理完毕。"NOR"\n");
	...
	while (i--)
	{
		...
		if (file[i][1] != -2)
		{
			...
			if(sscanf(file[i][0],"%s.o",filename)==1)
			{
				...
				ob = new("/clone/user/login");
				ob->set("id",filename+ZJGAMEPART);
				ob->restore();
				ob->set("id",filename+ZJGAMEPART);
				ob->save();
				destruct(ob);
