...
void kickout_player(object me, object ob)
{
	...
	log_file("cmds/adm/kickout_player", sprintf("%s %-9s kickout %s\n",
			log_time(), geteuid(me), geteuid(ob)));

	tell_object(me, "你把" + ob->query("name") + "强制退出了游戏！\n");
	tell_object(ob, "你被" + me->query("name") + "强制退出了游戏！\n");
//	CHANNEL_D->do_channel(this_object(), "rumor",sprintf("%s被%s强制退出了游戏。",ob->name(), me->name()));
	seteuid(ROOT_UID);
	if (ob->is_chatter())
	{
