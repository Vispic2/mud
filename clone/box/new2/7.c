...
void create()
{
        set_name(HIR"七彩神丸"NOR, ({ "qicai wan", "wan" }) );
        set_weight(500);
        if( clonep() )
...
//                return notify_fail("你正忙着哪。\n");
        ...
        if( !objectp(ob = present("qicai wan", me)) )
                return notify_fail("你没有这种东西。\n");

        message_vision(HIR"\n$n"HIR"一口吃下了$N,"HIR" $n觉的自己无比的强大!!!无人能敌!!  \n"NOR,this_object(),me);
me->add("qi",500);
me->add("eff_qi",500);
...
me->set_temp("apply/intelligence", 200);
me->set_temp("apply/strength",200);
me->set_temp("apply/dexerity",200);
me->set_temp("apply/constitution",200);
...
message_vision(HIY"$N觉的自己无比的强大!!无人能敌!!\n"NOR, me);
