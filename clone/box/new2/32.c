...
void create()
{
        set_name(HIR"遂发火枪"NOR, ({ "suifa huoqiang", "huoqiang" }) );
        set_weight(500);
        if( clonep() )
...
if (userp(me) && userp(target) && target->query("combat_exp",1) < me->query("combat_exp",1)) 
	return notify_fail("对方比你弱小，没必要使用！\n");
        ...
        if( !objectp(ob = present("suifa huoqiang", me)) )
...
if (userp(target))
{
target->set("qi",target->query("max_qi")/10);
if (target->query("eff_qi") > target->query("max_qi")/2)
target->set("eff_qi",target->query("max_qi")/2);
target->set("neili",target->query("neili")/10);
}
else
{
target->add("qi",-60000);
target->add("eff_qi",-50000);
}
