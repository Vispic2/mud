...
void create()
{
        set_name(HIR"真!天仙丸"NOR, ({ "zhentianxian wan", "wan" }) );
        set_weight(500);
        if( clonep() )
...
//                return notify_fail("你正忙着哪。\n");
        ...
        if( !objectp(ob = present("zhentianxian wan", me)) )
                return notify_fail("你没有这种东西。\n");

        message_vision(HIR"\n$n"HIR"一口吃下了$N,"HIR" $n觉的自己有了无限的超级精力!!  \n"NOR,this_object(),me);
me->add("qi",500);
me->add("eff_qi",500);
...
me->set("eff_qi",me->query("max_qi"));
	...
	me->set("jing",120000);
me->set("eff_jing",120000);       

message_vision(HIR"$N觉的自己有了无限的超级精力!!\n"NOR, me);
