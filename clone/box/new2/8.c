...
void create()
{
        set_name(HIR"五彩神丸"NOR, ({ "wucai wan", "wan" }) );
        set_weight(500);
        if( clonep() )
...
//                return notify_fail("你正忙着哪。\n");
        ...
        if( !objectp(ob = present("wucai wan", me)) )
                return notify_fail("你没有这种东西。\n");

        message_vision(HIR"\n$n"HIR"一口吃下了$N,"HIR" $n觉的自己很强大!!!无人能敌!!  \n"NOR,this_object(),me);
me->add("qi",500);
me->add("eff_qi",500);
...
me->set_temp("apply/intelligence", 100);
me->set_temp("apply/strength",100);
me->set_temp("apply/dexerity",100);
me->set_temp("apply/constitution",100);
me->set_temp("apply/karey", 100);
me->set_temp("apply/percao", 100);	
me->set_temp("apply/armor", 2000);
me->set_temp("apply/defense", 100);
me->set_temp("apply/attack", 100);
me->set_temp("apply/damage", 2000);        

message_vision(HIR"$N觉的自己很强大!!无人能敌!!\n"NOR, me);
