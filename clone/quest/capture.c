...
*/
	...
	if (! objectp(NPC3))
		return CYN "挂了，全死光了。" HIY + NPC3_NAME +
...
string query_introduce(object knower)
{
	...
	if (! objectp(NPC3))
		return CYN "嘿嘿，这下人死光了，也算是太平了。\n店"
		       "小二白眼一翻道：你算算，先是" HIY + NPC1_NAME
		       + NOR + CYN "，然后是" HIY + NPC2_NAME + NOR
		       + CYN "，最后又是" HIY + NPC3_NAME + NOR + CYN
		       "。\n" + knower->name() + CYN "又接着道：啧啧"
		       "啧，谁叫他们把该惹的和不该惹的都惹完了啊，这"
		       "下都把命出脱就高兴了。" NOR;
}
