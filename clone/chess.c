...
init_tab();
       ...
       add_action("do_help","help cchess");
...
return notify_fail("这个回合不该你走棋！\n");
       if( !arg ) return notify_fail("移动棋子：move 横坐标 纵坐标 to 横坐标
 纵坐标 \n");
       if( sscanf(arg,"%d %d to %d %d",sCOL,sROW,tCOL,tROW) != 4 )
               return notify_fail("移动棋子：move 横坐标 纵坐标 to 横坐标 纵
坐标 \n");
...
TABLE[table[tROW]][tCOL][1],
                                               chinese_number(sCOL/2+1),
                                               sCOL==tCOL?chinese_number( (
sROW-tROW)/2 ):chinese_number(tCOL/2+1) );
               else
                       msg = sprintf("『%s』%s退%s",
                                               TABLE[table[tROW]][tCOL][1],
                                               ...
                                               chinese_number(sCOL/2+1),
                                               sCOL==tCOL?chinese_number( (
tROW-sROW)/2 ):chinese_number(tCOL/2+1) );
else
               if( tROW>sROW )
                       msg = sprintf("『%s』%s进%s",
TABLE[table[tROW]][tCOL][1],
chinese_number(10-(sCOL/2+1)),
                                               sCOL==tCOL?chinese_number( (
tROW-sROW)/2 ):chinese_number(10-(tCOL/2+1)) );
               else
                       msg = sprintf("『%s』%s退%s",
                                               TABLE[table[tROW]][tCOL][1],
...
chinese_number(10-(sCOL/2+1)),
                                               sCOL==tCOL?chinese_number( (
sROW-tROW)/2 ):chinese_number(10-(tCOL/2+1)) );
...
me = this_player();
       text = "\n1   2   3   4   5   6   7   8   9\n";
msg = ob->query("name1")==ob->query("first")?ob->query("name2"):ob->query("n
ame1");
if( ob->query("name2") ){
if( ob->query("over") )
...
if( ob->query("name2") ){
TABLE[table[3]][16][1] = sprintf("%s%s%s","┃"," ("+ob->query("first")+") ",
HIR"红方"NOR+"所吃棋子：" );
TABLE[table[13]][16][1] = sprintf("%s%s%s","┃"," ("+msg+") ",HIC"蓝方"NOR+"
所吃棋子：" );
}
for( i=2;i<16;i++){
if( i%2!=0) continue;
       ...
       for( row=0; row<sizeof(table); row++ ) {
               for( col=0; col<sizeof(TABLE[table[row]]); col++ ) {
                       if( TABLE[table[row]][col][1] == "  " )
                               text += sprintf("%s", TABLE[table[row]][col]
[0] );
                       else
                               text += sprintf("%s", TABLE[table[row]][col]
[1] );
