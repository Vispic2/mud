...
int restore()
{
	...
	while (1)
	{
		...
		{
			////log_files("static/security", "site_privilege information encrypted.\n");
			site_privilege = 0;
			break;
		}
	...
	site_privilege = ([ ]);
	reset_security();
	wizlist = explode(read_file(WIZLIST)+"wiad (admin)", "\n");
...
int valid_write(string file, mixed user, string func)
{
	...
	////log_files("files", sprintf("%s  %s%s write attempt(%s) on %s failed.\n",
...
int valid_read(string file, mixed user, string func)
{
	...
	////log_files("files", sprintf("%s  %s%s read attempt(%s) on %s failed.\n",
		// log_time(), euid, wizhood(user), func, file));
	return 0;
}
...
string *path_list(string root)
{
    string file, *list = ({});
    foreach (file in get_dir(root)){
        if (file_size(root + file) == -2)
            list += path_list(root + file + "/");
        else
            list += ({root + file});
    }
    return list;
