...
void create()
{
    ...
    set("channel_id", "外敌入侵守护进程");
    CHANNEL_D->do_channel(this_object(),"sys", "外敌入侵精灵系统开始运行。");
    set("id", "invasion");
    call_out("main", 600);
}
...
void main()
{
    ...
    message( "chat", HIR"【外敌入侵】" HBRED "警报！警报！异族联军攻入北京了！\n" NOR
            HIR"【外敌入侵】"NOR HIY"请各位大侠速至北京抵御外敌！"+
            "。"NOR"\n", users() );
    //QQ_D->msg("【外敌入侵】警报！警报！异族联军攻入北京了！\n【外敌入侵】请各位大侠速至北京抵御外敌！");
...
object create_invader(int lvl, int number)
{
  ...
  //  });
    ...
    case "日本":
        ...
        NPCS_D->generate_jp_name(ob);
        break;
    case "英国":
    case "美国":
        ob = new(INVADE_NPC + "/english");
        NPCS_D->generate_e_name(ob);
        break;
    case "法国":
    case "德国":
    case "俄罗斯":
...
void set_reward()
{
    ...
    /*
    message( "channel:chat", HIR"【外敌入侵】"NOR HIG"大内总管[Danei zongguan]：圣旨到！"NOR"\n",users());
    message( "channel:chat",HIY"|------------------------------奉天承运，皇帝诏曰：------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
    message( "channel:chat",HIY"|---------------本次入侵之外敌被我方义士全歼，皇上龙颜大乐，-------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
    message( "channel:chat",HIY"|---------------今特命所有抗击外敌有功者进宫领赏。-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
    message( "channel:chat",HIY"|--------------------------------------------钦此！-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
*/
    message( "channel:chat", HIR"【外敌入侵】"NOR HIG"大内总管[Danei zongguan]：圣旨到！"NOR"\n",users());
    message( "channel:chat",HIY"|-------------奉天承运，皇帝诏曰：------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
    message( "channel:chat",HIY"|------本次入侵之外敌被我方义士全歼，皇上龙颜大乐，-------------------------------------------------------------------------------------------------------------------------------------------------------|"NOR"\n",users());
