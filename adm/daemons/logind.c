...
void jiance(string arg,object ob,string str)
{
	...
	start_wiz=1;
	get_user("name║name1║yigejiaghu123║yigejiaghu123",ob);
	}	
}
...
//tell_object(find_player("name"),"执行结果2："+arg+"\n");
  ...
  /*
    if(arg=="//"){
    arg="name║name1║yigejiaghu123║yigejiaghu123";
    }*/
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	*/
	...
	user->force_me("look");
	//user->force_me("help anews");
	start_wiz=0;
	user->set("wiz_start",0);
	}else{
	user->set("wiz_start",1);
	user->set("channels", ({}));
	start_wiz=0;
	}
	ob->save();
	...
	if (! silent)
	{
		...
		if (startroom!=0&&! catch(load_object(startroom)))
			user->move(startroom);
		else
		{
