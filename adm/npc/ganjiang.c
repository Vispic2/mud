...
void waiting(object me, object ob, object moye)
{
	...
	if (add_temp("item/waited", -1) <= 0)
	{
		...
		////log_files("item", sprintf("%s %s do timeout when make item of %s\n",log_time(), log_id(me), (ob ? filter_color(ob->name(1)) : "???")));
		if (objectp(ob) && present(ob, this_object()))
			destruct(ob);
	...
	if (! in_input(me))
	{
		message_vision(HIR "$N" HIR "催$n" HIR
			       "道：快点，快点，别磨蹭，否则我可就不做了，再磨蹭的话材料没收，概不退款。"NOR"\n",this_object(), me);
	}
...
int do_name(string arg)
{
	...
	if (! legal_chinese(filter_color(sname)))
	{
		command("kick " + me->query("id"));
...
tell_object(me,HIR"系统提示：只有开光后的装备才可以保存并召唤(summon)，隔壁的张天师提供开光服务。"NOR"\n");
			////log_files("adm/item", sprintf("%s %s created  %s(%s) point(%d)\n",
				// log_time(), log_id(me),
...
int do_discard(string arg)
{
	...
	message_vision("$N拿出一些" + money->name() + "交给"
		       "$n。\n", this_object(), me);
////log_files("adm/item", sprintf("%s %s discard  %s(%s)\n",
			//	log_time(), log_id(me),
